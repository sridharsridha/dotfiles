...
class MacOSSetup(Printer):
    ...
    def setup_ssh(self):
        ...
        if not ssh_key_path.exists():
            ...
            self.run_command(["ssh-keygen", "-t", "ed25519", "-C", "sridha.in@gmail.com", "-f", str(ssh_key_path), "-N", ""], check=False)
        else:
        ...
        if not ssh_config_path.exists() or "Host github.com" not in ssh_config_path.read_text():
        ...
        self.run_command(["ssh-add", "--apple-use-keychain", str(ssh_key_path)], check=False)
...
class DotfilesManager(Printer):
    ...
    def resolve_path(self, path_str: str) -> Path:
        return Path(path_str.replace("$HOME", str(self.home))
                            .replace("$CONFIG", str(self.home / ".config"))
                            .replace("$PWD", str(self.pwd)))
    ...
    def create_symlink(self, source: str, target: str):
        ...
        if self.dry_run:
            self.print_dry_run(f"Would create parent directory for {target_path} if it doesn't exist.")
            if target_path.exists() or target_path.is_symlink():
                self.print_dry_run(f"Would move existing {target_path.name} to {target_path.name}.bkp")
            self.print_dry_run(f"Would link {source_path} to {target_path}")
        else:
            if target_path.exists() or target_path.is_symlink():
                self.print_warning(f"Moving existing {target_path.name} to {target_path.name}.bkp")
    ...
    def install_fonts(self):
        ...
        if not font_dir.is_dir():
            self.print_warning(f"Font directory '{font_dir}' not found. Skipping font installation.")
            return
    ...
    def process_apps(self, apps: List[str], action: str):
        ...
        for app_name in apps:
            ...
            if "check" in config:
                ...
                if check["type"] == "cmd_exists" and not self.cmd_exists(check["cmd"]):
                    self.print_warning(f"Command '{check['cmd']}' not found. Skipping {app_name}.")
                    continue
                if check["type"] == "dir_exists" and not path.is_dir():
                    self.print_warning(f"Directory '{path}' not found. Skipping {app_name}.")
                    continue
            ...
            for step in steps:
                ...
                if step_action == "symlink":
                    ...
                    command = [self.resolve_path(c) if isinstance(c, str) and "$" in c else c for c in step["command"]]
    ...
    def run(self):
        ...
        group.add_argument("-i", "--install", nargs='*', choices=app_choices, help="Install dotfiles for specific applications.\nIf no apps are specified, all apps will be installed.")
        group.add_argument("-d", "--delete", nargs='*', choices=app_choices, help="Delete dotfiles for specific applications.\nIf no apps are specified, all apps will be deleted.")
        parser.add_argument("--setup-macos", action="store_true", help="Run macOS setup commands.")
